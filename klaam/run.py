...
from klaam.external.FastSpeech2.inference import prepare_tts_model, infer_tts
...
class TextToSpeech:
    ...
    def __init__(self, prepare_tts_model_path, model_config_path, train_config_path, vocoder_config_path=None, speaker_pre_trained_path=None):
        self.prepare_tts_model = yaml.load(
            open(prepare_tts_model_path, "r"), Loader=yaml.FullLoader
        )
        ...
        self.model, self.vocoder, self.configs = prepare_tts_model(self.configs, self.vocoder_config_path, self.speaker_pre_trained_path)
